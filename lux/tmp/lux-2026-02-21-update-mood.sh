...
if command -v sensors &> /dev/null; then
    # Get max temp from all sensors (extract °C values)
    MAX_TEMP=$(sensors | grep -oP '\+\K[0-9.]+(?=°C)' | sort -rn | head -1 || echo "0")
else
    MAX_TEMP="unknown"
fi
...
# Move to final location (requires sudo or appropriate perms)
if [[ -w "$MOOD_FILE" ]]; then
    mv "$OUTPUT" "$MOOD_FILE"
elif command -v sudo &> /dev/null; then
    sudo mv "$OUTPUT" "$MOOD_FILE"
else
    echo "Warning: Cannot write to $MOOD_FILE (no write permission and sudo unavailable)" >&2
    exit 1
fi
